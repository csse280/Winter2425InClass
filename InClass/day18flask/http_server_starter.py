...
# TODO 0: Confirm that you successfully ran `pip install flask` per the assigned reading.
# TODO 1: Replace all the HTTP server code below the handle special 
#         routes function with one line to construct the equivalent 
#         Flask HTTP server.
# TODO 2: Convert /API/LOAD to its own Flask function.
# TODO 3: Convert /API/SAVE to its own Flask function.
# TODO 4: Delete handle_special_routes()
# TODO 5: Actually RUN Flask. Test that the server still works.
def handle_special_routes(file_name, post_data):
    ...
    if ("/API/LOAD" in file_name):
        print("/API/LOAD invoked")
        data = json.dumps( shopping_list )
        special_response_body = bytearray(data, encoding='utf-8')
        special_content_type = 'text/javascript; charset=utf-8'
...
# Flask does everything past this line for us.
